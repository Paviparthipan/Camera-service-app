    ...
    return (
        <div>
                ...
                {togBill &&
                    <div className='fixed   transition inset-0 bg-opacity-50  bg-black/65 flex items-center justify-center'>
                        <div className='h-screen relative  bg-white w-full p-4 m-15 '>
                            <button onClick={togglebill} className='absolute top-4 hover:bg-red-400 px-3 rounded-2xl py-1 hover:text-white font-semibold  right-5'>x</button>

                            <h2 className=' text-center font-semibold  mt-10 text-lg'> Bill List</h2>
                                ...
                                <tbody>
                                    {bills.map((item) => (
                                        <tr key={item._id}>
                                            ...
                                            <td className="border
                                             border-gray-400 
                                             px-4 py-2" onClick={() => toggleOPenBill(item._id)} ><button className='bg-blue-600 px-4 py-1 rounded text-white hover:bg-blue-500'>open</button></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
            ...
            <div>
                ...
                {openBill && <div className='fixed   transition inset-0 bg-opacity-50  bg-black/65 flex items-center justify-center'   >

                    <div className='relative bg-white h-auto w-auto'>
                        <button onClick={toggleOPenBill} className='absolute top-4 hover:bg-red-400 px-3 rounded-2xl py-1 hover:text-white font-semibold  right-5'>x</button>


                        <div className='b-white p-10 overflow-auto'>
                            <h2 className='text-center text-2xl font-semibold'>Tax Invoice</h2>
